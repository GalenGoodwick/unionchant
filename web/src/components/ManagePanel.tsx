...
export default function ManagePanel({ onOpenChant, onOpenGroup }: {
  onOpenChant?: (id: string) => void
            ...
            <div key={c.id} className="p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <a
                    href={`/chants?dock=${c.id}`}
                    onClick={e => { if (onOpenChant) { e.preventDefault(); onOpenChant(c.id) } }}
                    className="text-xs text-foreground hover:text-accent font-medium line-clamp-2 cursor-pointer"
                  >
                    {c.question}
                  </a>
          ...
          {groups.map(g => (
            <div key={g.id} className="p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <a
                    href={`/chants?dock=group:${g.slug}`}
                    onClick={e => { if (onOpenGroup) { e.preventDefault(); onOpenGroup(g.slug) } }}
                    className="text-xs text-foreground hover:text-accent font-medium cursor-pointer"
                  >
                    {g.name}
                  </a>
                  <div className="flex items-center gap-1.5 mt-1 text-xs text-muted">
