      ...
      {activeField && (
        <div className="pointer-events-auto mb-2 bg-surface/95 backdrop-blur border border-border rounded-lg p-3 w-72 max-h-64 overflow-y-auto">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-muted">{activeField.shape?.type === 'circle' ? `circle r=${activeField.shape.radius}` : activeField.shape?.type === 'rect' ? `rect ${activeField.shape.w}x${activeField.shape.h}` : 'no shape'}</span>
          </div>
