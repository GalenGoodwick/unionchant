// MARIONETTES ULTRA — the quantization comes off.
...
fn mod_lum2(n: vec3f) -> vec2f {
  let key = normalize(vec3f(-0.5, -0.85, 0.55));
  let fill = normalize(vec3f(0.6, 0.7, 0.45));
  let lum = max(dot(n, key), 0.0) * 0.85 + max(dot(n, fill), 0.0) * 0.25 + 0.16;
  let spec = pow(max(dot(n, normalize(key + vec3f(0.0, 0.0, 1.0))), 0.0), 32.0);
  return vec2f(lum, spec);
}`

const ARENA = /* wgsl */`
fn visual_mar4_arena(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
...
// ── Serpent ──
const SERPENT = /* wgsl */`
fn mar4_serpent_sd(q: vec2f, hd: f32, ph: f32, ra: f32, reach: f32) -> vec2f {
...
fn visual_mar4_serpent(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = uv * 22.0;
  let hd = p.x; let ph = p.y; let ra = p.z; let reach = clamp(p.w, 0.0, 1.0);
  let res = mar4_serpent_sd(c, hd, ph, ra, reach);
  let d = res.x;
  let alpha = smoothstep(0.18, -0.18, d);
  if (alpha <= 0.004) {
    let ds = mar4_serpent_sd(c - vec2f(3.5, 4.5), hd, ph, ra, reach).x;
    let sh = smoothstep(2.2, -1.2, ds) * 0.40;
    return vec4f(vec3f(0.0), sh);
  }
  let e = 0.25;
  let dx = mar4_serpent_sd(c + vec2f(e, 0.0), hd, ph, ra, reach).x - d;
  let dy = mar4_serpent_sd(c + vec2f(0.0, e), hd, ph, ra, reach).x - d;
  let n = normalize(vec3f(-dx / e, -dy / e, 1.35));
  let ls = mod_lum2(n);
...
// ── Walker ──
const WALKER = /* wgsl */`
fn mar4_walker_sd(q: vec2f, hd: f32, ph: f32) -> f32 {
...
fn visual_mar4_walker(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = uv * 22.0;
  let hd = p.x; let ph = p.y; let reach = clamp(p.w, 0.0, 1.0);
  let d = mar4_walker_sd(c, hd, ph);
  let alpha = smoothstep(0.18, -0.18, d);
  if (alpha <= 0.004) {
    let ds = mar4_walker_sd(c - vec2f(3.5, 4.5), hd, ph);
    let sh = smoothstep(2.2, -1.2, ds) * 0.40;
    return vec4f(vec3f(0.0), sh);
  }
  let e = 0.25;
  let dx = mar4_walker_sd(c + vec2f(e, 0.0), hd, ph) - d;
  let dy = mar4_walker_sd(c + vec2f(0.0, e), hd, ph) - d;
  let n = normalize(vec3f(-dx / e, -dy / e, 1.35));
  let ls = mod_lum2(n);
...
// ── Puppet ──
const PUPPET = /* wgsl */`
fn mar4_puppet_sd(q: vec2f, hd: f32, ph: f32, ra: f32, reach: f32) -> f32 {
...
fn visual_mar4_puppet(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = uv * 22.0;
  let hd = p.x; let ph = p.y; let ra = p.z; let reach = clamp(p.w, 0.0, 1.0);
  let d = mar4_puppet_sd(c, hd, ph, ra, reach);
  let alpha = smoothstep(0.18, -0.18, d);
  if (alpha <= 0.004) {
    let ds = mar4_puppet_sd(c - vec2f(3.5, 4.5), hd, ph, ra, reach);
    let sh = smoothstep(2.2, -1.2, ds) * 0.40;
    return vec4f(vec3f(0.0), sh);
  }
  let e = 0.25;
  let dx = mar4_puppet_sd(c + vec2f(e, 0.0), hd, ph, ra, reach) - d;
  let dy = mar4_puppet_sd(c + vec2f(0.0, e), hd, ph, ra, reach) - d;
  let n = normalize(vec3f(-dx / e, -dy / e, 1.35));
  let ls = mod_lum2(n);
...
// ── Crawler ──
const CRAWLER = /* wgsl */`
fn mar4_crawler_sd(q: vec2f, hd: f32, ph: f32) -> f32 {
...
fn visual_mar4_crawler(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = uv * 22.0;
  let hd = p.x; let ph = p.y; let reach = clamp(p.w, 0.0, 1.0);
  let d = mar4_crawler_sd(c, hd, ph);
  let alpha = smoothstep(0.18, -0.18, d);
  if (alpha <= 0.004) {
    let ds = mar4_crawler_sd(c - vec2f(3.5, 4.5), hd, ph);
    let sh = smoothstep(2.2, -1.2, ds) * 0.40;
    return vec4f(vec3f(0.0), sh);
  }
  let e = 0.25;
  let dx = mar4_crawler_sd(c + vec2f(e, 0.0), hd, ph) - d;
  let dy = mar4_crawler_sd(c + vec2f(0.0, e), hd, ph) - d;
  let n = normalize(vec3f(-dx / e, -dy / e, 1.35));
  let ls = mod_lum2(n);
...
const BRIDGE = /* wgsl */`
fn visual_mar4_bridge(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
...
const scene = {
  name: 'MARIONETTES ULTRA',
  fields: [
    field('mar4_arena_f', 'Arena', [0.04, 0.05, 0.08, 1], 256, 256, { shapeType: 'rect', w: 512, h: 512 }, 'mar4_arena'),
    field('mar4_bridge_1', 'Bridge 1', [0.7, 1, 1, 1], 30, 30, { shapeType: 'circle', radius: 96 }, 'mar4_bridge', [0, 0, 0, 0]),
    field('mar4_bridge_2', 'Bridge 2', [0.7, 1, 1, 1], 30, 60, { shapeType: 'circle', radius: 96 }, 'mar4_bridge', [0, 0, 0, 0]),
    field('mar4_serpent_f', 'Serpent', [0.2, 0.6, 0.4, 1], 150, 150, { shapeType: 'circle', radius: 100 }, 'mar4_serpent', [0, 0, 0, 0]),
    field('mar4_walker_f', 'Walker', [0.78, 0.52, 0.18, 1], 370, 150, { shapeType: 'circle', radius: 100 }, 'mar4_walker', [0, 0, 0, 0]),
    field('mar4_puppet_f', 'Puppet', [0.75, 0.3, 0.46, 1], 150, 370, { shapeType: 'circle', radius: 100 }, 'mar4_puppet', [0, 0, 0, 0]),
    field('mar4_crawler_f', 'Crawler', [0.42, 0.44, 0.85, 1], 370, 370, { shapeType: 'circle', radius: 100 }, 'mar4_crawler', [0, 0, 0, 0]),
  ],
  worldParams: { gravity: 0, friction: 1.0, collisionForce: 0, boundaryMode: 'solid', bounciness: 0.5, gravitationalConstant: 0 },
  worldData: {
    postProcess: { bloomIntensity: 0.18, bloomThreshold: 0.85, exposure: 1.05, vignetteStrength: 0.25, vignetteRadius: 0.8 },
  },
  ...
  interactionEffects: [],
  visualTypes: [
    { name: 'mar4_arena', wgsl: ARENA },
    { name: 'mar4_bridge', wgsl: BRIDGE },
    { name: 'mar4_serpent', wgsl: SERPENT },
    { name: 'mar4_walker', wgsl: WALKER },
    { name: 'mar4_puppet', wgsl: PUPPET },
    { name: 'mar4_crawler', wgsl: CRAWLER },
  ],
  modules: [{ name: 'skel4', wgsl: MODULES }],
  timestamp: Date.now(),
}
...
const res = await fetch('http://localhost:3000/api/engine/scene', {
  ...
  body: JSON.stringify({ action: 'save', name: 'MARIONETTES ULTRA', scene }),
})
console.log('MARIONETTES ULTRA saved:', res.status, await res.text())
