// MARIONETTES II (detail pass) — the skeleton rig becomes flesh, then character.
...
const ARENA = /* wgsl */`
fn visual_mar2_arena(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let t = (uv * 0.5 + vec2f(0.5)) * 128.0;
  let q = floor(t);
  var c = vec3f(0.016, 0.019, 0.031);
  c += vec3f(0.010, 0.012, 0.020) * (q.y / 128.0);
  let f = fbm(q * 0.045, 3);
...
const SERPENT = /* wgsl */`
fn mar2_serpent_sd(q: vec2f, hd: f32, ph: f32, ra: f32, reach: f32) -> vec2f {
...
fn visual_mar2_serpent(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = mod_px2(uv, 44.0);
  let hd = p.x; let ph = p.y; let ra = p.z; let reach = clamp(p.w, 0.0, 1.0);
  let res = mar2_serpent_sd(c, hd, ph, ra, reach);
  let d = res.x;
  if (d > 1.0) {
    // dithered contact shadow, cast down-right of the body
    let ds = mar2_serpent_sd(c - vec2f(4.0, 5.0), hd, ph, ra, reach).x;
    if (ds < 0.0 && mod_chk2(c) > 0.5) { return vec4f(0.004, 0.005, 0.010, 0.6); }
    return vec4f(0.0);
  }
  if (d > 0.0) { return vec4f(0.02, 0.08, 0.05, 1.0); }
  let e = 0.9;
  let dx = mar2_serpent_sd(c + vec2f(e, 0.0), hd, ph, ra, reach).x - d;
  let dy = mar2_serpent_sd(c + vec2f(0.0, e), hd, ph, ra, reach).x - d;
  let n = normalize(vec3f(-dx, -dy, 1.1));
  let L = normalize(vec3f(-0.5, -0.85, 0.62));
  let lum = clamp(dot(n, L) * 0.75 + 0.35, 0.0, 1.0);
  let band = mod_band2(lum, c, 4.0);
  ...
  if (mod_blink2(time, 0.13) < 0.5) {
    color = mix(color, vec3f(0.02, 0.04, 0.03), mod_node2(c, eyeP, 0.7));
    color = mix(color, vec3f(0.95), mod_node2(c, eyeP + vec2f(1.0, -1.0), 0.4));
  }
...
const WALKER = /* wgsl */`
fn mar2_walker_sd(q: vec2f, hd: f32, ph: f32) -> f32 {
...
fn visual_mar2_walker(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = mod_px2(uv, 44.0);
  let hd = p.x; let ph = p.y; let reach = clamp(p.w, 0.0, 1.0);
  let d = mar2_walker_sd(c, hd, ph);
  let bob = sin(ph * 2.0) * 0.8;
  let head = rotate(vec2f(13.0, -3.0 + bob), hd);
  ...
  if (d > 1.0 && earA + earB < 0.5) {
    let ds = mar2_walker_sd(c - vec2f(4.0, 5.0), hd, ph);
    if (ds < 0.0 && mod_chk2(c) > 0.5) { return vec4f(0.004, 0.005, 0.010, 0.6); }
    return vec4f(0.0);
  }
  if (d > 0.0 && earA + earB < 0.5) { return vec4f(0.10, 0.05, 0.02, 1.0); }
  let e = 0.9;
  let dx = mar2_walker_sd(c + vec2f(e, 0.0), hd, ph) - d;
  let dy = mar2_walker_sd(c + vec2f(0.0, e), hd, ph) - d;
  let n = normalize(vec3f(-dx, -dy, 1.1));
  let L = normalize(vec3f(-0.5, -0.85, 0.62));
  let lum = clamp(dot(n, L) * 0.75 + 0.35, 0.0, 1.0);
  let band = mod_band2(lum, c, 4.0);
  ...
  let speck = step(0.84, hash21(c + vec2f(37.0, 11.0)));
  ...
  if (mod_blink2(time, 0.47) < 0.5) {
    color = mix(color, vec3f(0.06, 0.03, 0.01), mod_node2(c, eyeP, 0.7));
    color = mix(color, vec3f(0.95), mod_node2(c, eyeP + vec2f(1.0, -1.0), 0.4));
  }
...
const PUPPET = /* wgsl */`
fn mar2_puppet_sd(q: vec2f, hd: f32, ph: f32, ra: f32, reach: f32) -> f32 {
...
fn visual_mar2_puppet(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = mod_px2(uv, 44.0);
  let hd = p.x; let ph = p.y; let ra = p.z; let reach = clamp(p.w, 0.0, 1.0);
  let d = mar2_puppet_sd(c, hd, ph, ra, reach);
  let bob = sin(ph * 2.0) * 0.6;
  let head = rotate(vec2f(1.4, -11.0 + bob), hd);
  ...
  if (d > 1.0 && hair < 0.5) {
    let ds = mar2_puppet_sd(c - vec2f(4.0, 5.0), hd, ph, ra, reach);
    if (ds < 0.0 && mod_chk2(c) > 0.5) { return vec4f(0.004, 0.005, 0.010, 0.6); }
    return vec4f(0.0);
  }
  if (d > 0.0 && hair < 0.5) { return vec4f(0.10, 0.02, 0.05, 1.0); }
  let e = 0.9;
  let dx = mar2_puppet_sd(c + vec2f(e, 0.0), hd, ph, ra, reach) - d;
  let dy = mar2_puppet_sd(c + vec2f(0.0, e), hd, ph, ra, reach) - d;
  let n = normalize(vec3f(-dx, -dy, 1.1));
  let L = normalize(vec3f(-0.5, -0.85, 0.62));
  let lum = clamp(dot(n, L) * 0.75 + 0.35, 0.0, 1.0);
  let band = mod_band2(lum, c, 4.0);
  ...
  let pelvis = rotate(vec2f(0.0, 2.0 + bob), hd);
  let chest = rotate(vec2f(0.8, -5.0 + bob), hd);
  let vest = step(mod_seg2(c, pelvis, chest), 2.1) * mod_chk2(c);
  ...
  if (mod_blink2(time, 0.71) < 0.5) {
    color = mix(color, vec3f(0.05, 0.01, 0.03), mod_node2(c, eyeP, 0.7));
    color = mix(color, vec3f(0.95), mod_node2(c, eyeP + vec2f(1.0, -1.0), 0.4));
  }
  return vec4f(color, 1.0);
}`
...
const CRAWLER = /* wgsl */`
fn mar2_crawler_sd(q: vec2f, hd: f32, ph: f32) -> f32 {
...
fn visual_mar2_crawler(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
  let c = mod_px2(uv, 44.0);
  let hd = p.x; let ph = p.y; let reach = clamp(p.w, 0.0, 1.0);
  let d = mar2_crawler_sd(c, hd, ph);
  if (d > 1.0) {
    let ds = mar2_crawler_sd(c - vec2f(4.0, 5.0), hd, ph);
    if (ds < 0.0 && mod_chk2(c) > 0.5) { return vec4f(0.004, 0.005, 0.010, 0.6); }
    return vec4f(0.0);
  }
  if (d > 0.0) { return vec4f(0.05, 0.05, 0.14, 1.0); }
  let e = 0.9;
  let dx = mar2_crawler_sd(c + vec2f(e, 0.0), hd, ph) - d;
  let dy = mar2_crawler_sd(c + vec2f(0.0, e), hd, ph) - d;
  let n = normalize(vec3f(-dx, -dy, 1.1));
  let L = normalize(vec3f(-0.5, -0.85, 0.62));
  let lum = clamp(dot(n, L) * 0.75 + 0.35, 0.0, 1.0);
  let band = mod_band2(lum, c, 4.0);
  ...
  let spot = step(0.90, hash21(c + vec2f(53.0, 29.0)));
  ...
  color = mix(color, vec3f(0.03, 0.03, 0.08), mod_node2(c, eyeP, 0.7));
  color = mix(color, vec3f(0.95), mod_node2(c, eyeP + vec2f(1.0, -1.0), 0.4));
  return vec4f(color, 1.0);
}`
...
const BRIDGE = /* wgsl */`
fn visual_mar2_bridge(uv: vec2f, sdf: f32, col: vec4f, time: f32, p: vec4f, behind: vec4f) -> vec4f {
...
const scene = {
  name: 'MARIONETTES II',
  fields: [
    field('mar2_arena_f', 'Arena', [0.02, 0.02, 0.04, 1], 256, 256, { shapeType: 'rect', w: 512, h: 512 }, 'mar2_arena'),
    field('mar2_bridge_1', 'Bridge 1', [0.7, 1, 1, 1], 30, 30, { shapeType: 'circle', radius: 96 }, 'mar2_bridge', [0, 0, 0, 0]),
    field('mar2_bridge_2', 'Bridge 2', [0.7, 1, 1, 1], 30, 60, { shapeType: 'circle', radius: 96 }, 'mar2_bridge', [0, 0, 0, 0]),
    field('mar2_serpent_f', 'Serpent', [0.2, 0.6, 0.4, 1], 150, 150, { shapeType: 'circle', radius: 66 }, 'mar2_serpent', [0, 0, 0, 0]),
    field('mar2_walker_f', 'Walker', [0.78, 0.52, 0.18, 1], 370, 150, { shapeType: 'circle', radius: 66 }, 'mar2_walker', [0, 0, 0, 0]),
    field('mar2_puppet_f', 'Puppet', [0.75, 0.3, 0.46, 1], 150, 370, { shapeType: 'circle', radius: 66 }, 'mar2_puppet', [0, 0, 0, 0]),
    field('mar2_crawler_f', 'Crawler', [0.42, 0.44, 0.85, 1], 370, 370, { shapeType: 'circle', radius: 66 }, 'mar2_crawler', [0, 0, 0, 0]),
  ],
  ...
  interactionEffects: [],
  visualTypes: [
    { name: 'mar2_arena', wgsl: ARENA },
    { name: 'mar2_bridge', wgsl: BRIDGE },
    { name: 'mar2_serpent', wgsl: SERPENT },
    { name: 'mar2_walker', wgsl: WALKER },
    { name: 'mar2_puppet', wgsl: PUPPET },
    { name: 'mar2_crawler', wgsl: CRAWLER },
  ],
...
const res = await fetch('http://localhost:3000/api/engine/scene', {
  ...
  body: JSON.stringify({ action: 'save', name: 'MARIONETTES II', scene }),
})
console.log('MARIONETTES II (detail pass) saved:', res.status, await res.text())
