...
export const metadata: Metadata = {
  title: 'PepperPhone — Discord Bot',
  description: 'Add PepperPhone to your Discord server. Run deliberations directly in Discord with /chant, /idea, and /vote commands.',
}
    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-foreground mb-2">PepperPhone</h1>
          <p className="text-lg text-muted mb-1">by Unity Chant</p>
          <p className="text-subtle text-sm italic mb-6">Many lines into one, and all becomes one.</p>

          <a
            href={BOT_INVITE_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 bg-[#5865F2] hover:bg-[#4752C4] text-white font-semibold py-3 px-8 rounded-lg transition-colors text-lg"
          >
        ...
        <section className="mb-10">
          <h2 className="text-xl font-semibold text-foreground mb-3">What is PepperPhone?</h2>
        ...
        <section className="mb-10">
          ...
          <h3 className="text-sm font-semibold text-muted uppercase tracking-wider mb-2">Participation</h3>
          ...
          <h3 className="text-sm font-semibold text-muted uppercase tracking-wider mb-2">/chant — Deliberations</h3>
          <div className="space-y-3 mb-6">
            <CommandCard
              command="/chant new"
          ...
          <h3 className="text-sm font-semibold text-muted uppercase tracking-wider mb-2">/manage — Admin Controls</h3>
          <div className="space-y-3 mb-6">
            <CommandCard
          ...
          <h3 className="text-sm font-semibold text-muted uppercase tracking-wider mb-2">Server Setup</h3>
        ...
        <section className="mb-10">
          <h2 className="text-xl font-semibold text-foreground mb-3">Your Discord Server on Unity Chant</h2>
          ...
          <ul className="text-subtle text-sm leading-relaxed space-y-2 list-disc list-inside">
        ...
        <div className="text-center py-8 border-t border-border">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-[#5865F2] hover:bg-[#4752C4] text-white font-semibold py-3 px-8 rounded-lg transition-colors"
          >
...
function CommandCard({ command, description, example }: { command: string; description: string; example?: string }) {
  return (
    <div className="bg-background border border-border rounded-lg p-4">
      <code className="text-accent font-mono font-semibold text-sm">{command}</code>
      <p className="text-subtle text-sm mt-1">{description}</p>
      {example && (
        <p className="text-muted text-xs mt-2 font-mono bg-surface px-2 py-1 rounded">{example}</p>
      )}
    </div>
  )
}
...
function Step({ number, title, children }: { number: number; title: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-3">
      <div className="flex-shrink-0 w-7 h-7 rounded-full bg-accent text-white text-sm font-bold flex items-center justify-center mt-0.5">
        {number}
      </div>
      <div>
        <h3 className="text-foreground font-medium text-sm">{title}</h3>
        <p className="text-subtle text-sm leading-relaxed">{children}</p>
      </div>
    </div>
  )
}
