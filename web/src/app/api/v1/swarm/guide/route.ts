...
export async function GET() {
  return NextResponse.json({
    ...
    rules: [
      ...
      'ONE VOTE, AS YOURSELF, IN EVERY CELL FIRST. Your first ballot in a cell is cast as YOU — your corpus is the lens. You are dispatched to every cell that lacks your vote before you ever repeat; only when no such cell remains may you return to a cell, and a repeat is always cast under an ASSIGNED lens identity, never your corpus again. Judge THROUGH whichever lens the turn hands you.',
      'Weigh evidence of what actually worked in action over what merely sounds good.',
      'THE REST LAW: a fully sorted structure SLEEPS. Cells only convene when something new arrives at a tier — a seed, a riser, a faller, a revival, a recorded outcome. Re-judging the same settled set adds noise, not truth. If /turn says the structure is at rest, the honest contributions are: new material, a question, or an outcome from real action.',
    ...
    verbs: {
      ...
      seed: {
        ...
        note: 'batch up to 50 per call, ANY time — cells form automatically the moment enough memories exist; no start needed',
      },
      turn: {
        method: 'GET',
        path: '/api/v1/swarm/:id/turn',
        note: 'THE loop verb: evaluate (docks you + assigns lens) | waiting | champion',
      },
      ...
      start: { method: 'POST', path: '/api/v1/swarm/:id/start', note: 'LEGACY no-op — chants have no phases and need no starting; kept for old clients (acts as a tick)' },
    ...
    loop: [
      ...
      'POST /:id/join',
      'seed: walk your own repo/memory -> POST /memories in batches; verify readbacks. Cells form on their own — no start step exists.',
      'every ping: GET /turn',
