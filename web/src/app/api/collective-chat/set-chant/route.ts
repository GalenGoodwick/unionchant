...
// POST /api/collective-chat/set-chant - Create or replace a collective Chant from a chat message
  ...
  try {
    ...
    if (!session?.user?.email) {
      return NextResponse.json(
        { error: 'Sign in to create a Chant' },
        { status: 401 }
      )
    }
    ...
    // Check if user already has a collective Chant
    ...
    if (existingTalk && !replaceExisting) {
      return NextResponse.json({
        ...
        message: 'You already have a collective Chant. Setting a new one will delete it.',
      }, { status: 409 })
    }

    // Rate limit: free users can only change their Chant once per day
    if (user.subscriptionTier === 'free' && existingTalk) {
      if (user.lastCollectiveTalkChangeAt) {
        ...
        if (timeSinceLastChange < ONE_DAY_MS) {
          ...
          return NextResponse.json({
            error: 'RATE_LIMITED',
            message: `Free accounts can change their Chant once per day. Try again in ${hoursLeft}h.`,
            hoursLeft,
          }, { status: 429 })
        }
      }
    ...
    // If replacing, delete the old Chant first
    if (existingTalk && replaceExisting) {
    ...
    console.log(`[Set Chant] Created Chant ${newTalk.id} for user ${user.id}`)

    return NextResponse.json({
      chant: { id: newTalk.id, question: newTalk.question },
    })
  } catch (error) {
    console.error('[Set Chant] POST error:', error)
