...
export async function GET() {
  try {
    ...
    const result = await cached(`user:${email}`, 10_000, async () => {
      ...
      // All counts in single parallel batch
      const [ideasCount, votesCount, commentsCount, deliberationsCreatedCount, membershipsCount, xpResult, adminFlag] = await Promise.all([
...
export async function PATCH(request: Request) {
  try {
    ...
    invalidate(`user:${session.user.email}`)
