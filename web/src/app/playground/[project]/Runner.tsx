...
export default function Runner({ project }: { project: string }) {
  ...
  useEffect(() => {
    const load = () =>
      fetch(`/api/playground?project=${project}`)
        .then((r) => r.json())
        .then((d) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground px-4 py-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-1">
        <Link href="/playground" className="text-muted-light hover:text-foreground text-xs font-mono">← workshop</Link>
        <label className="flex items-center gap-1.5 font-mono text-[11px] text-muted-light cursor-pointer">
          <input type="checkbox" checked={autoRun} onChange={(e) => setAutoRun(e.target.checked)} />
          auto-run on change
        </label>
      </div>
      <h1 className="font-serif text-2xl mb-1">{project}</h1>
      <p className="text-muted text-sm mb-5">
        ...
        <span className="font-mono text-xs text-muted-light">agents: GET /api/v1/playground for the contract · this project: ?project={project}</span>
      </p>
      ...
      {nodes === null ? (
        ...
        <div className="rounded-lg border border-border bg-header/40 p-4 font-mono text-xs text-muted-light">
          empty project — the first agent to POST {'{'}action:&quot;create&quot;, project:&quot;{project}&quot;{'}'} writes node one.
        </div>
      ) : (
