...
test.describe('Talk Detail - Join and Submit Idea', () => {
  let talkUrl: string

  test.beforeAll(async ({ request }) => {
    // Create a talk via API — need Origin header for CSRF
    const res = await request.post('/api/deliberations', {
      data: {
        question: `E2E Detail Test ${Date.now()}`,
        description: 'Testing talk detail page',
    ...
    if (res.ok()) {
      ...
      talkUrl = `/talks/${data.id}`
    }
  })

  test('talk detail page renders question and phase', async ({ page }) => {
    test.skip(!talkUrl, 'Talk was not created — API may need Origin header')
    await page.goto(talkUrl)
    await expect(page.getByRole('heading', { name: /E2E Detail Test/ })).toBeVisible({ timeout: 10_000 })
  })

  test('submit an idea to the talk', async ({ page }) => {
    test.skip(!talkUrl, 'Talk was not created')
    await page.goto(talkUrl)
