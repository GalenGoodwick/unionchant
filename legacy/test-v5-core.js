// Union Chant v5 - Verification Tests (Batch Mode)
// Run with: node test-v5.js
...
async function testInvariant3_CrossCellTallying() {
    ...
    // Start voting to form Tier 1 cells
    await startVoting();
...
async function testCorrectFlow_15Participants() {
    ...
    const tier2Ideas = state.ideas.filter(i => i.tier === 2);
    if (tier2Ideas.length !== 3) {
        console.log(`   ❌ FAIL: Expected 3 advancing ideas, got ${tier2Ideas.length}`);
        return false;
    }
    console.log(`   ✓ ${tier2Ideas.length} ideas advancing to Tier 2`);
...
async function testFlexibleCellSizing() {
    console.log('\n✅ TEST 5: Flexible Cell Sizing');
    console.log('   System handles any participant count correctly\n');

    // Test with 17 participants (should form [5, 5, 7])
    await reset();
    console.log('   Testing 17 participants (expected [5, 5, 7])...');
    ...
    const expectedSizes = [5, 5, 7];

    console.log(`   Cell sizes: [${cellSizes.join(', ')}]`);

    if (JSON.stringify(cellSizes) !== JSON.stringify(expectedSizes)) {
        console.log(`   ❌ FAIL: Expected [${expectedSizes.join(', ')}], got [${cellSizes.join(', ')}]`);
        return false;
    }

    // Verify no invalid sizes
    const invalidSizes = cellSizes.filter(s => s === 1 || s === 2 || s >= 8);
    if (invalidSizes.length > 0) {
        console.log(`   ❌ FAIL: Found invalid cell sizes: ${invalidSizes.join(', ')}`);
        return false;
    }

    console.log(`   ✓ Cell sizes correct: [${cellSizes.join(', ')}]`);
    console.log('   ✅ PASS: Flexible cell sizing works correctly');
    return true;
}

async function runAllTests() {
    console.log('╔════════════════════════════════════════╗');
    console.log('║  Union Chant v5 - Verification Tests  ║');
    console.log('╚════════════════════════════════════════╝');
