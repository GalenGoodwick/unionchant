...
function autoVote(req, res) {
    ...
    req.on('end', () => {
        ...
        if (cell.status !== 'voting') {
            res.writeHead(400);
            res.end(JSON.stringify({ error: 'Cell is not in voting state' }));
            return;
        }
        ...
        availableParticipants.forEach(participantId => {
            // Randomly select an idea from the cell
        ...
        console.log(`🤖 Auto-voted ${votesAdded} times in ${cellId}`);
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: true, votesAdded }));
    });
}
...
function completeTier(req, res) {
    ...
    req.on('end', () => {
        ...
        // Accept: 3, 5, 7, or any number >= 7
        const PRIME_THRESHOLDS = [3, 5, 7];
        const shouldAdvance = PRIME_THRESHOLDS.includes(completedCells.length) || completedCells.length >= 7;
        ...
        if (tier === 1) {
            // Tier 1: Each cell had different ideas, winners already added to ideas array
            advancingIdeas = ideas.filter(i => i.tier === tier + 1);
            console.log(`   ${advancingIdeas.length} ideas advancing to Tier ${tier + 1}`);
        } else {
            // Tier 2+: Cross-cell tallying (INVARIANT #3)
            console.log(`   📊 Cross-cell tallying for Tier ${tier}...`);
            ...
            console.log(`   Tally:`, crossCellTally);

            // Find ONE winner (INVARIANT #5)
            const winner = Object.keys(crossCellTally).reduce((a, b) =>
                crossCellTally[a] > crossCellTally[b] ? a : b
            );

            console.log(`   🏆 Winner: ${winner} with ${crossCellTally[winner]} total votes`);

            // Check if this is the final winner
            console.log(`\n🎉 FINAL WINNER: ${winner}`);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({
                winner: { id: winner, totalVotes: crossCellTally[winner] },
                message: 'Winner declared!'
            }));
            return;
        }

        // Only Tier 1 reaches here - form Tier 2 cells
